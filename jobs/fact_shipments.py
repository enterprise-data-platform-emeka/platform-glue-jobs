...
clean_df.unpersist()
print(f"[fact_shipments] Done. {silver_row_count:,} rows written.")

publish_row_count_metric("fact_shipments", silver_row_count, args["JOB_NAME"])
publish_freshness_metric("fact_shipments", max_dms_ts, args["JOB_NAME"])
commit_job(job)
