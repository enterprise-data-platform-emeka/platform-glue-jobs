...
clean_df.unpersist()
print(f"[fact_payments] Done. {silver_row_count:,} rows written.")

publish_row_count_metric("fact_payments", silver_row_count, args["JOB_NAME"])
publish_freshness_metric("fact_payments", max_dms_ts, args["JOB_NAME"])
commit_job(job)
